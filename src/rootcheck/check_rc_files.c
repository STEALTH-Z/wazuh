...
void check_rc_files(const char *basedir, FILE *fp)
{
    ...
    while (fgets(buf, OS_SIZE_1024, fp) != NULL) {
        ...
        if (*file == '/') {
#endif
            bytes_wrotes = snprintf(file_path, OS_SIZE_1024, "%s", file);
        } else {
            bytes_wrotes = snprintf(file_path, OS_SIZE_1024, "%s%c%s", basedir, PATH_SEP, file);
        }

       if (bytes_wrotes + 1 > OS_SIZE_1024) {
